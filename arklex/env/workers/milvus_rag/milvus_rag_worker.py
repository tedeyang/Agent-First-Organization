...
class MilvusRAGWorker(BaseWorker):
    ...
    def _execute(self) -> MilvusRAGWorkerOutput:
        print(
            "orch state stream type: ",
            self.milvus_rag_worker_data.orch_state.stream_type,
        )
        retrieved_text, retriever_params = RetrieveEngine.milvus_retrieve(
            self.milvus_rag_worker_data.orch_state.user_message.history,
            self.milvus_rag_worker_data.orch_state.bot_config,
            self.milvus_rag_worker_data.tags,
        )
        self.milvus_rag_worker_data.orch_state.message_flow = retrieved_text
        # state = trace(input=retriever_params, state=state)
        if self.milvus_rag_worker_data.orch_state.stream_type != StreamType.NON_STREAM:
            response = ToolGenerator.stream_context_generate(
                self.milvus_rag_worker_data.orch_state
            )
        else:
            response = ToolGenerator.context_generate(
                self.milvus_rag_worker_data.orch_state
            )
