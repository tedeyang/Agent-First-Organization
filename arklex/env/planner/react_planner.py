...
class ReactPlanner(DefaultPlanner):
    ...
    def _format_tool_info(self, tools_map: Dict[str, Any]) -> Dict[str, PlannerResource]:
        ...
        tools = [tool["execute"]() for tool in self.tools_map.values()]
        formatted_tools_info = {}
        for tool_class in tools:
            tool_info = tool_class.info
            tool_outputs = tool_class.output
            ...
            if tool_info["type"] == "function":
                ...
                required = tool_info["function"]["parameters"]["required"]
    ...
    def _get_planning_trajectory_summary(self, state: MessageState, msg_history) -> str:
        """
    ...
    def _get_num_resource_retrievals(self, summary: str) -> int:
        ...
        valid_summary = True
        try:
            steps = self._parse_trajectory_summary_to_steps(summary)
            n_steps = len(steps)

            if n_steps == 0:
                valid_summary = False
            else:
                n_retrievals = NUM_STEPS_TO_NUM_RETRIEVALS(n_steps)
    ...
    def _retrieve_resource_signatures(
        ...
        planning_steps = trajectory_summary
        steps = self._parse_trajectory_summary_to_steps(trajectory_summary)
        if len(steps) > 0:
    ...
    def message_to_actions(
        ...
        resource_id = self.name2id.get(resource_name, None)
        ...
        if resource_id is not None and (
            ...
            args = message.get("arguments", None)
    ...
    def step(self, action: Action, msg_state: MessageState) -> EnvResponse:
        ...
        # tools_map indexed by tool ID
            ...
            try:
                resource_id = self.name2id[action.name]
                calling_tool = self.tools_map[resource_id]
        ...
        # workers_map indexed by worker name
            ...
            try:
                worker = self.workers_map[action.name]["execute"]()
                observation = worker.execute(msg_state)
                if not isinstance(observation, str):
                    # Convert to string if not already
                    observation = str(observation)
