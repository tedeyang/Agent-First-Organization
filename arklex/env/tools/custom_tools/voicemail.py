...
def _end_call_thread(twilio_client: TwilioClient, call_sid: str, response_played_event: threading.Event):
    """Helper function to end the call in a separate thread."""
    try:
        logger.info(f"Ending call with call_sid: {call_sid}. Sleeping for 10 seconds to allow for final answer")
        time.sleep(20)
        logger.info(f"Ending call with call_sid: {call_sid}. Waiting for response to be played")
...
def voicemail(**kwargs):
    ...
    threading.Thread(target=_end_call_thread, args=(twilio_client, call_sid, response_played_event)).start()
