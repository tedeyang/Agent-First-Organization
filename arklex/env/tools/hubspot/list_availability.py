...
description = "List the availability of the company representative from Husbspot calendar"
...
def list_availability(timezone: str, duration:int, **kwargs):
    ...
    available_times = ""
    try:
        for avail_time in res["linkAvailability"]["linkAvailabilityByDuration"][str(meeting_duration_ms)]["availabilities"]:
            # convert the timestamp to the user's timezone
            avail_time_utc = datetime.fromtimestamp(avail_time["startMillisUtc"] / 1000, tz=pytz.utc)
