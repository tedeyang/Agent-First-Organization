...
def make_doc_missing_title(doc):
    d = doc.copy()
    d.pop("title", None)
    return d


def make_doc_invalid_sections_type(doc):
    d = doc.copy()
    d["sections"] = "not a list"
    return d


def make_doc_invalid_section(doc):
    d = doc.copy()
    d["sections"][0] = {"name": "Section 1"}  # missing content
    return d


def make_doc_missing_section_field(doc):
    d = doc.copy()
    d["sections"][0] = {"content": "no name"}
    return d


def make_doc_duplicate_section_names(doc):
    d = doc.copy()
    d["sections"][1]["name"] = "Section 1"
    return d


def make_doc_empty_requirements(doc):
    d = doc.copy()
...
class TestDocumentValidator:
    ...
    def test_validate_structure_valid(self, validator, valid_document) -> None:
        """Test valid document structure."""
        assert validator.validate_structure(valid_document) is True

    def test_validate_structure_missing_field(self, validator, valid_document) -> None:
    ...
    def test_validate_required_fields_valid(self, validator, valid_document) -> None:
    ...
    def test_validate_consistency_valid(self, validator, valid_document) -> None:
    ...
    def test_get_error_messages_all_valid(self, validator, valid_document) -> None:
    ...
    def test_get_error_messages_consistency(self, validator, valid_document) -> None:
        """Test error messages for consistency issues."""
        doc = make_doc_duplicate_section_names(valid_document)
        errors = validator.get_error_messages(doc)
        assert "Document consistency issues" in errors

    def test__validate_section_valid(self, validator) -> None:
