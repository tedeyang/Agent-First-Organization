...
class TestCheckAvailable:
    ...
    def test_check_available_success_with_same_date_slots(
        ...
        # Use a timestamp that corresponds to January 1st in the current year
        from datetime import datetime

        current_year = datetime.now().year
        jan_1_timestamp = int(datetime(current_year, 1, 1, 10, 0, 0).timestamp() * 1000)
        jan_1_end_timestamp = int(
            datetime(current_year, 1, 1, 10, 15, 0).timestamp() * 1000
        )
